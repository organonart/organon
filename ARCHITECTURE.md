...
- `$TMPDIR/organic-math-glyphs.bin` (organon#217 T1 — `organon-core/src/glyph_ring.rs`,
  ...
  instanced, bevelled, **emissive** tile (`doc/pbr_text_engine.md`) — and, under T9's
  `LowerOptions::dark_tiles` (`glyph_dark_tiles`, default off, on `Shared.glyph[14]`), **every**
  cell, the symbol-less ones as dark quarter-depth tiles at exactly zero emission that show
  only the faceplate's sheen; T12's `LowerOptions::motion` (`Slide` today, `Exact`, `Cells`;
  wire pending on `Shared.glyph[15]`) chooses whether a path character slides between its
  two exact positions, sits at this tick's, or sits at its cell centre — a teleport cuts
  under all three. The slide's clock is `glyph_ring::BlendClock` (W16): the pair's span is
  `Δtick / tick_hz` from the frame, the lead is the world's own frame interval (a frame
  built now is shown one interval later), and a heartbeat — same `tick` — replaces the
  current grid without rotating the previous one or restarting the clock; nothing new
  on the wire. A SEPARATE mmap channel on
...
| **`organon-render`** | `native/organon-render` | `organon-core`, `wgpu`, `glam`, `bytemuck`, `image`, `half` | **T4** — the renderer: `render` + its 36 surface submodules, plus `axes`/`chamber`, **`legibility`** (PBR text T2 — the CPU harness, no wgpu in it), **`legibility_gate`** + the `legibility-gate` `[[bin]]` (T13 — that harness as a command `verify.sh --legibility` runs) and **50 shaders**. **No nih-plug, no egui, no winit.** |
...
| `organon-core/src/glyph_ring.rs` | organon#217 T1 — the **glyph ring**: `GlyphCell` (32 B, offsets pinned by test) / `GlyphFrame` / `GlyphRingHeader` + `GlyphRingWriter`/`GlyphRingReader` (double buffer with a lap guard; layout-version + cell-stride refusal), the symbol→tile table `tile_for` (block/shade glyphs → sub-cell extent + extrusion depth; unknown → full block at reduced emission), `srgb8_to_linear` and its exact inverse `linear_to_srgb8` (T11 — the producer decays in linear and the ring stays sRGB8), the `SGR_PERSIST` trail bit, and `lower_grid` — grid → instances/tints/**emits** + backplane, each tile at its cell centre **plus the cell's `sub_x`/`sub_y`** and `active_path` cells sliding between the previous and current *exact* positions, never from a trail (+ tests). **T9:** `lower_grid_with(…, LowerOptions)` — `dark_tiles` lowers every symbol-less cell as a full-cell tile at the `░` depth (`DARK_TILE`), faceplate tint, emit exactly zero, at its cell centre (never sliding — a space *character* on a path is dark and still), bounds untouched; default off and byte-identical to `lower_grid`; the world calls `lower_grid_with` with the flag read off `Shared.glyph[14]` (`glyph_dark_tiles`). **T12:** `LowerOptions::motion: Motion` — `Slide` (the default: today's lerp between exact positions on `ACTIVE_PATH`, a cut otherwise), `Exact` (no inter-tick interpolation; `centre + sub` this tick), `Cells` (cell centres, remainder ignored); `Slide` ≡ `Exact` at `blend ≥ 1`, pinned; `Motion::from_lane` is the proposed `Shared.glyph[15]` mapping (garbage → `Slide`). **W16:** `Arrival` / `classify_arrival` / `blend_for` / `BlendClock` — the world's inter-tick blend as a clock on **producer** time (the pair's span is `Δtick / tick_hz`; `tick` is the producer's clock within an epoch) with the world's own frame interval as the lead, pinned on 120/60, 30/120, 60/60 and 90/60 schedules over synthetic seconds (steps of exactly 2, 0.25, 1 and 1.5 ticks a frame), a heartbeat pinned not to restart it, a stall pinned to clamp at 1. Replaces the world's `since × tick_hz` from the read, which drew a 120 Hz producer two ticks behind on a 60 Hz display and never between. In core because the writer (`organon-glyphs`) and the reader (`world.rs`) must share ONE definition and core is the only crate both see |
...
| `surface_format.rs` | organon#237 — **which swapchain format to configure, decided from what the surface offers *now***, pure and tested. The visual died twice mid-run in `Surface::configure` re-issuing an `Rgba16Float` it had read at first light: on Vulkan the format list is a live property of the display the window is on (fp16 + extended-linear only while that output is in HDR mode), so a monitor waking in SDR or the window landing on the other display removed it, and a validation error in `configure` is a panic by default. `pick_surface_format(want_hdr, offered)` → fp16 when wanted and offered; else the first **sRGB** format (the SDR path's own choice — HDR-off is byte-identical); else the first offered — and it is called from **`WindowSurface::configure` in `main.rs`, the one place the swapchain is configured** (first light, the **H** toggle, a resize, a lost/outdated acquire), inside error scopes so a failure is logged and the frame loop skips rather than the process dying. `hdr_active` records the grant and gates the headroom read, the layer tag and the `HDR output:` lines. ⚠️ `Rgb10a2Unorm` is deliberately not in the ladder: `composite.wgsl`'s SDR arm writes linear and relies on the surface's sRGB OETF, and there is no `Rgb10a2UnormSrgb`. `doc/arch/render.md`'s seam section carries the full account |
...
| `rt_caustic.rs` | #258 T5 **photon-mapped caustics** — the GPU side of the light-tracing pass: clears a per-pixel fixed-point splat buffer, then traces photons from the key light — and, while a glyph ring is live, from the lit tiles too (organon#217 T8b: a per-frame power CDF over `emits`, no parameter) |
...
| `organon-render/src/legibility_gate.rs` + `src/bin/legibility_gate.rs` | **PBR text T13 (organon#217)** — the **legibility gate**: T2's harness as a thing a person re-runs. A **thresholds file** (`native/verify/legibility/thresholds.toml`, all four keys required, an unknown key an error), **`locate`** (a centred scale sweep then an offset refinement maximising Pearson — the held-camera frame is not quite `GridGeom::fit`'s case, because the rig fits the bounds' centre plane and the tiles' front faces are nearer), the **fixture from the ring** (`fixture_from_grid` + `shape_agrees`: colours from the settled `GlyphGrid`, shape cross-checked against the file cell for cell; an unsettled ring is refused), the **spread** between two frames of one held render (the determinism claim as a number), `emit_text` (the producer's input derived from the fixture, so there is no second copy of the logo), and the exit code (0 pass · 1 a threshold not met · 2 could not measure, never conflated). The `legibility-gate` `[[bin]]` is a thin `main`; `verify.sh --legibility` runs it over two `organon snap`s of `faceplate` on the Omarchy logo (`verify/legibility/faceplate.scene` — as much of the rung as the CLI vocabulary reaches). ⚠️ It scores the frame `snap` writes: 8-bit sRGB **after the visual's Reinhard tonemap**, so a gain above 1 is compressed rather than clipped or linear; the report's first line says so. `tests/legibility_gate.rs` drives the module and the built binary over the synthetic painter and a ring written to a temp file. → `doc/arch/render.md` "The legibility harness", `native/verify/README.md` |
