...
>   auto-injected.**
> - `web/ARCHITECTURE.md` — the **web app** (`web/`). Authoritative on it; §18 here is
>   only a pointer.
> - `CLAUDE.md` — project context, conventions, the toolchain, the macOS/Ableton
>   workflow. It deliberately does **not** describe architecture; that is this file.
> - `STATUS.md` — the volatile session handoff, and **the current test counts**.
> - `CHANGELOG.md` — per-PR history, and where the `Shared` layout's accretion story
>   lives (it is a ledger, not architecture — organon#590).
> - `.claude/skills/organon-dev` — the *process* above all of these.
> - `doc/organic_math_paper*.md` — the essays (the *why*; Part III is a roadmap).
>
...
web/                    ⏸ PARKED — the WebGPU port (#418) → web/ARCHITECTURE.md
src/                    the LEGACY React-Three-Fiber app (§18)
site/  site-mind/       organon.art · organonmind.org (static, Vercel)
original_code/          the original C++ (algorithm source of truth)
doc/                    PRDs, the papers, build plans, shop notes, status archive
scripts/                the public-mirror generator + site/essay generators
skills/                 organon-cli (symlinked into .claude/skills)
brand/  songs/

ARCHITECTURE.md (this file)  ·  MIND_ARCHITECTURE.md  ·  CLAUDE.md
...
| `cli.rs` / `bin/ctl.rs` | #452 the `organon` CLI: arg parsing + catalog/status/get/watch formatting + `CliOp` op building (pure, tested) / the thin bin (mmap reads, command-channel appends, exit codes) |
