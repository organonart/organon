...
struct Uniforms {
    ...
    // pass never reads it — a missed reflection ray returns 0 so the env/IBL reflection
    // stands, and a reflection of the environment is reflected LIGHT, not the backdrop
    // the camera sees. Hiding the backdrop leaves the IBL alone in the raster too, so
