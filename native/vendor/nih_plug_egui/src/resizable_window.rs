...
use crate::egui::{pos2, CentralPanel, Context, Frame, Id, Rect, Response, Sense, Ui, Vec2};
use crate::EguiState;
...
impl ResizableWindow {
    ...
    /// Override the wrapped `CentralPanel`'s frame.
    ///
    /// **Added for organon#593 Tier 4.** The default (`None`) is `CentralPanel::default()`,
    /// byte-for-byte what this wrapper has always drawn — an opaque faceplate filled with
    /// `Visuals::panel_fill`. Organon Mind's wgpu editor renders the 3-D world into the same
    /// surface *before* egui's pass, so there the central region has to let that through; it
    /// passes a transparent frame here. Every other host passes nothing and is unaffected.
    #[inline]
    pub fn frame(mut self, frame: impl Into<Option<Frame>>) -> Self {
    ...
    pub fn show<R>(
        ...
        let panel = match self.frame {
            Some(frame) => CentralPanel::default().frame(frame),
            None => CentralPanel::default(),
        };
