...
// ORGANON PATCH (#554): the one symbol this file takes from ours. Defined (and tested) in
// `lib.rs` so the decision itself, which is Organon's and not upstream's, stays out of the
// otherwise-verbatim file. See its use in the fragment entry-point choice below.
use crate::is_linear_target;
...
/// ORGANON ADDITION (#554) — not upstream, and not a wgpu-30 port fix.
///
/// The pipeline build, lifted out of [`Renderer::new`] verbatim so that
/// [`Renderer::set_target_format`] can rebuild **only** the pipeline when the swapchain
/// format changes, instead of the caller throwing the whole `Renderer` away. See that
/// method for why discarding the `Renderer` is a crash rather than a cost.
fn build_pipeline(
    device: &wgpu::Device,
    ...
    let depth_stencil = options
        ...
        device.create_render_pipeline(&wgpu::RenderPipelineDescriptor {
            ...
            vertex: wgpu::VertexState {
                ...
                module: &module,
                // ORGANON PATCH (#554): wgpu 30 made vertex buffer layouts optional, so a
                // pipeline can leave a slot unbound. egui binds slot 0; the layout is
                // otherwise unchanged.
            ...
            fragment: Some(wgpu::FragmentState {
                module: &module,
                // ORGANON PATCH (#554): upstream picks the entry point on `is_srgb()`
                // alone, which silently mis-encodes the UI on Organon's **HDR/EDR path**.
                //
                // The real question is not "is this sRGB?" but "does this target hold
                // LINEAR values?", and there are two ways to answer yes:
                //   - an `*Srgb` format: linear in the buffer, hardware applies the OETF
                //     on write;
                //   - a **float** format (`Rgba16Float`, which `bin/visual.rs` swaps to for
                //     macOS EDR, tagged `extendedLinearSRGB`/`extendedLinearITUR_2020`):
                //     linear in the buffer, and no hardware OETF at all.
                // `Rgba16Float.is_srgb()` is `false`, so upstream would take the gamma
                // branch and write sRGB-encoded values into a linear buffer — a washed-out,
                // too-bright UI that appears *only* when HDR output is on, and only on a
                // display with EDR headroom. Exactly the failure no test here can see.
                entry_point: Some(if is_linear_target(output_color_format) {
            ...
            // ORGANON PATCH (#554): renamed in wgpu 30. `None` carries the same meaning
            // (no multiview), and matches how `render.rs` spells it.
            multiview_mask: None,
            cache: None,
        }
    )
...
pub struct Renderer {
    ...
    // ORGANON ADDITION (#554): kept so `set_target_format` can rebuild the pipeline in
    // place. Both are cheap handles, and holding them is what lets the texture map and
    // every bind group survive an HDR toggle.
    pipeline_layout: wgpu::PipelineLayout,
...
impl Renderer {
    ...
    pub fn new(
        ...
        let pipeline_layout = device.create_pipeline_layout(&wgpu::PipelineLayoutDescriptor {
            ...
            // ORGANON PATCH (#554): wgpu 30 made pipeline-layout bind groups optional
            // (`Option<&BindGroupLayout>`, so a layout can leave a group index unbound), and
            // replaced `push_constant_ranges` with `immediate_size`. egui uses neither
            // push constants nor immediate data, so the size is 0.
            bind_group_layouts: &[
        ...
        // ORGANON PATCH (#554): `depth_stencil` moved into `build_pipeline` alongside the
        // rest of the descriptor, so a format change rebuilds it too. Its wgpu-30 fixes
        // (`depth_write_enabled` / `depth_compare` became `Option`) travelled with it.
        ...
        Self {
            pipeline,
            // ORGANON ADDITION (#554) — see `set_target_format`.
            pipeline_layout,
    ...
    /// ORGANON ADDITION (#554) — not upstream, and not a wgpu-30 port fix.
    ///
    /// Rebuild the pipeline for a new target format, **keeping everything else**.
    ///
    /// Organon's swapchain format is not fixed: toggling HDR output swaps the surface
    /// between an SDR `*Unorm` format and `Rgba16Float`, and the pipeline is bound to both
    /// the format and — via [`crate::is_linear_target`] — the fragment entry point, so it
    /// genuinely has to be rebuilt.
    ///
    /// **Replacing the whole `Renderer` instead is a crash, not merely waste.** A fresh
    /// `Renderer` starts with an empty texture map, while the `egui::Context` on the other
    /// side is untouched and still believes its font atlas is resident on the GPU. egui
    /// therefore sends the *next* atlas change as an incremental delta (`ImageDelta::pos`
    /// = `Some`), and [`Renderer::update_texture`] panics with "Tried to update a texture
    /// that has not been allocated yet." It does not fire immediately or reliably, which
    /// is what makes it nasty: with no UI on screen no text is laid out, so no delta is
    /// ever emitted and the toggle looks fine. It dies the first time the atlas changes
    /// with the panel up.
    ///
    /// Rebuilding in place keeps `textures`, `samplers` and every bind group — including
    /// their affinity to this renderer's own `texture_bind_group_layout`, so there is no
    /// question of layout compatibility between a carried-over bind group and a pipeline
    /// built from a different layout object.
    ///
    /// Cheap enough to call unconditionally, but the caller should still gate on an actual
    /// format change — see `ui_layer::UiLayer::set_format`.
            ...
            for epaint::ClippedPrimitive { primitive, .. } in paint_jobs {
                match primitive {
                    Primitive::Mesh(mesh) => {
                        ...
                        // ORGANON PATCH (#554): wgpu 30 dropped `DerefMut<[u8]>` on
                        // `QueueWriteBufferView` in favour of an explicitly write-only
                        // `.slice(range) -> WriteOnly<[u8]>`. Same copy, and the new form
                        // makes the write-only-ness of staging memory a type error rather
                        // than a comment.
            ...
            for epaint::ClippedPrimitive { primitive, .. } in paint_jobs {
                match primitive {
                    Primitive::Mesh(mesh) => {
                        ...
                        // ORGANON PATCH (#554): see the index-buffer note above.
