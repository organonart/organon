...
//!
//! ## organon#49 T4c-i — this is a crate now, and two functions did not come with it
//!
//! It was `native/src/agent.rs`. Its whole coupling to the plugin was **two functions**,
//! and both stayed behind in a root-crate `agent.rs` that re-exports everything here:
//!
//! | Stayed up | Because it reads |
//! |---|---|
//! | `core_catalog()` | `param_table::pack_*::catalog` — the plugin's automation surface |
//! | `scene_features()` | `preset::{PresetValues, PresetScope, EditorTab}` |
//!
//! ⚠️ **So `crate::agent::core_catalog` still resolves and no caller moved** — the module
//! path is unchanged, which is the `HostFuncName` shape organon#49 has now used five
//! times: the semantic half below, the host adapter above, meeting at one name.
//!
//! 📌 **Nothing here knows how a catalog is produced.** [`system_prompt`] and
//! [`catalog_prompt`] take `&[CatSlot]`, and `World::new` is handed one at construction.
//! That property is *why* this crate could exist, so keep it: a function here that
//! reaches for a catalog rather than receiving one puts the plugin back overhead.
//!
//! Pieces:
...
//! - [`CatSlot`] — one slot of the prompt-side vocabulary. The *list* is generated
//!   mechanically from the `param_block!` slot lists in `param_table.rs` by the root
//!   crate's `core_catalog`, so a param added to a core block appears automatically and
...
use organon_core::params::{GeneratorMode, IndexedEnum, MaterialType, SurfaceMode};
...
// ⚠️ `core_catalog()` IS NOT HERE — it is the root crate's `agent::core_catalog`.
//
// It is built from `param_table::pack_*::catalog`, and `param_table` is the plugin's own
// automation surface (3 734 lines, and it names *this* module back), so it could not
// descend. It stays in `native/src/agent.rs`, which re-exports everything below and adds
// it — the `HostFuncName` shape, semantic half here and host half above, at the same
// module path so no caller moved.
//
// Consumers here take a catalog as a **value**: `catalog_prompt` and `system_prompt` both
// receive `&[CatSlot]`, and `World::new` is handed one at construction. Nothing in this
// crate knows how a catalog is produced, which is exactly the property that let it move.
...
impl AgentConfig {
    ...
    pub fn load() -> AgentConfig {
        std::fs::read_to_string(organon_core::ipc::agent_config_path())
    ...
    pub fn save(&self) {
        let _ = std::fs::write(organon_core::ipc::agent_config_path(), self.to_body());
    }
}
...
/// fingerprint** (the root crate's `agent::scene_features`, which stayed up with `preset`
/// in organon#49 T4c-i): the distinguishing settings of *this* preset in the
...
mod tests {
    ...
    fn capability_catalog_covers_every_variant_and_is_bounded() {
        ...
        // organon#49 T4c-i: an EMPTY catalog, not `core_catalog()` — which is the root
        // crate's now. Both assertions read `capability_catalog()` and `look_notes()`,
        // neither of which consults the param catalog at all, so the test is measuring
        // exactly what it always was. Passing `&[]` makes that independence explicit
    ...
    fn actuatable_ids_is_exactly_the_id_range_set() {
        ...
        // ⚠️ The REVERSE direction — every catalog id with a range must be listed — is
        // `catalog_ids_with_a_range_are_all_actuatable` in the root crate's `agent.rs`.
        // organon#49 T4c-i split it there because it is the only half that needs
        // `core_catalog()`, which reads `param_table` and could not descend. Two
        // assertions, two reasons to fail, two homes — the same split T3 made for
        // `substrate_materials`. Change one and look at the other.
    }
