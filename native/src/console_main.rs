...
use organon_console::theme::Theme;
...
/// the variables below are a shipped flag surface, and `organon-console` is a *wire* identifier
...
/// is `command.rs`'s convention (`session.note`, `console.echo`) and what makes
...
/// service validates against the same list the renderer draws from. That is the console-side
...
/// CONSOLE_ARCHITECTURE.md §2 names; it is not in scope here and is not quietly half-done.
///
...
struct Console {
    ...
    /// reframe (PRD v3.2): the Console is a TUI HOST — the default tab runs the default
    ...
    pane_resized: bool,
    /// The `Shared` snapshot writer (organon-console namespace). In the two-process
    /// design the PLUGIN writes this; the Console has no plugin, so the terminal writes
...
impl Console {
    ...
    /// told. That gap is in `CONSOLE_ARCHITECTURE.md`'s honesty ledger.
    ///
    ...
    fn snapshot_live_backdrop(&mut self) -> Option<Rc<CachedEpoch>> {
        ...
        let (texture, view) = {
            ...
            let texture = device.create_texture(&wgpu::TextureDescriptor {
                label: Some("console-backdrop-epoch"),
            ...
            let view = texture.create_view(&wgpu::TextureViewDescriptor {
                label: Some("console-backdrop-epoch-sample"),
                format: Some(BACKDROP_SAMPLE_FORMAT),
                ..Default::default()
            });
            let mut encoder = device.create_command_encoder(&wgpu::CommandEncoderDescriptor {
                label: Some("console-backdrop-epoch-copy"),
            });
    ...
    fn render_backdrop(&mut self) -> Option<egui::TextureId> {
        ...
        if rebind {
            ...
            let view = texture.create_view(&wgpu::TextureViewDescriptor {
                label: Some("console-backdrop-sample"),
                format: Some(BACKDROP_SAMPLE_FORMAT),
                ..Default::default()
            });
    ...
    fn make_surface_texture(
        ...
        let texture = device.create_texture(&wgpu::TextureDescriptor {
            label: Some("console-conversation-surface"),
        ...
        let view = texture.create_view(&wgpu::TextureViewDescriptor {
            label: Some("console-conversation-surface-sample"),
            format: Some(BACKDROP_SAMPLE_FORMAT),
            ..Default::default()
        });
        let renderer = self.renderer.as_mut()?;
    ...
    fn redraw(&mut self) {
        ...
        let mut encoder = device
            .create_command_encoder(&wgpu::CommandEncoderDescriptor { label: Some("console-ui") });
        let mut staged = renderer.update_buffers(&device, &queue, &mut encoder, &jobs, &sd);
        {
            let mut rp = encoder
                .begin_render_pass(&wgpu::RenderPassDescriptor {
                    label: Some("console-ui-pass"),
...
/// `organon-console`). There was no argument handling at all: the flag was ignored, the banner
...
fn main() {
    ...
    event_loop.run_app(&mut console).expect("run app");
}
...
mod cli_tests {
    ...
    fn the_console_does_not_introduce_itself_as_the_shell() {
        ...
        assert!(!h.contains("organon-console "), "the usage line still names the old binary");
    ...
    /// **The console-side half of the name-list drift guard.** The `CommandService` schema and
