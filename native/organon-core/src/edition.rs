...
//! by the `mind-edition` / `console-edition` cargo features (mutually exclusive,
...
pub enum Edition {
    ...
    Mind,
    /// Organon Console — the agent-operating workstation (`--features console-edition`,
    /// Console #3 T1). Standalone-only like Mind: no plugin identity, ever.
    Console,
}
...
#[cfg(all(feature = "mind-edition", feature = "console-edition"))]
compile_error!(
    "`mind-edition` and `console-edition` are mutually exclusive — one build is one product"
);

/// The edition **this build** is. Selected by the `mind-edition` / `console-edition`
/// cargo features (default: neither → full Organon).
#[cfg(not(any(feature = "mind-edition", feature = "console-edition")))]
pub const EDITION: Edition = Edition::Full;

/// The edition **this build** is. Selected by the `mind-edition` cargo feature.
#[cfg(all(feature = "mind-edition", not(feature = "console-edition")))]
pub const EDITION: Edition = Edition::Mind;

/// The edition **this build** is. Selected by the `console-edition` cargo feature.
#[cfg(all(feature = "console-edition", not(feature = "mind-edition")))]
pub const EDITION: Edition = Edition::Console;
...
/// The `UiTab`s Organon Console would show (Console #3 T1).
///
/// **Provisional, and honestly so:** the Console's front-of-house is its own workspace
/// (the compositor in `native/organon-console`), not the plugin editor, so no Console
/// binary draws a tab bar today. The set still needs a defined answer — every
/// edition-shaped decision is a pure function of the `Edition` value — and the defensible
/// one is the look-shaping tabs an embedded editor pane would want: the same slice Mind
/// ships, minus the Mind lane. Revisit when a Console surface actually embeds the editor
/// UI.
const CONSOLE_TABS: &[UiTab] =
...
impl Edition {
    ...
    pub const fn product_name(self) -> &'static str {
        match self {
            ...
            Edition::Console => "Organon Console",
        }
    }

    /// One-line description of the product, for the Mind-edition header strip.
    ///
    /// **The Console's is deliberately empty** — the console says nothing about itself
    ...
    ///
    /// 🚨 **`Console` is frozen at `"organon-shell"` and survives the Console rename on
    /// purpose.** It is a **wire identifier, not a name**: the `organon` CLI joins on this
    /// exact string to find the running console's mmap and sidecars, and the launch shims
    /// on the workstation set `ORGANON_IPC_NS=organon-shell` to fork a second console into
    /// its own namespace. Both live outside this repo, so moving the string here does not
    /// move them — `organon console …` would simply stop reaching the console it is typed
    /// into, with no error on either side. The crate, the module, the feature and the
    /// `Edition` variant all renamed around it precisely because none of them is read by
    /// another process; this one is. Changing it needs a migration, not a find-and-replace.
    pub const fn ipc_namespace(self) -> &'static str {
        match self {
            Edition::Full => "organic-math",
            Edition::Mind => "organon-mind",
            Edition::Console => "organon-shell",
        }
    }
    ...
    pub fn shows_tab(self, tab: UiTab) -> bool {
        match self {
            ...
            Edition::Console => CONSOLE_TABS.contains(&tab),
        }
    }
    ...
    /// `true` for the Console edition (Console #3 T1) — the one-liner the
    /// workstation code branches on.
    pub const fn is_console(self) -> bool {
        matches!(self, Edition::Console)
    }
}
...
mod tests {
    ...
    /// Pairwise across all three — this is what lets a Console session, a Mind session
    /// and a full-Organon session run simultaneously.
    #[test]
    fn editions_have_distinct_ipc_namespaces() {
        let all = [Edition::Full, Edition::Mind, Edition::Console];
    ...
    fn namespaces_are_filename_safe() {
        for e in [Edition::Full, Edition::Mind, Edition::Console] {
    ...
    fn default_tab_is_always_visible() {
        ...
        assert_eq!(Edition::Console.default_tab(), UiTab::Look);
    }
    ...
    /// Organon Console's provisional tab answer (Console #3 T1): the look-shaping
    /// tabs, no Mind lane, no Generator/Synth/Audio — and its own name + namespace.
    ///
    /// 🚨 **The namespace assertion is the guard on the one string the Console rename
    /// deliberately left behind.** `"organon-shell"` is a wire identifier the `organon`
    /// CLI joins on from another process; this line is what makes a well-meaning
    /// find-and-replace fail here, loudly, instead of at a user's keyboard, silently.
    /// See `ipc_namespace`'s doc for the argument.
    #[test]
    fn console_edition_identity_and_tabs() {
        assert_eq!(Edition::Console.product_name(), "Organon Console");
        assert_eq!(Edition::Console.ipc_namespace(), "organon-shell");
        assert!(Edition::Console.is_console());
        assert!(!Edition::Console.is_mind());
        assert!(!Edition::Full.is_console());
        assert_eq!(
            Edition::Console.visible_tabs(),
            &[UiTab::Look, UiTab::Motion, UiTab::Environment, UiTab::Settings]
        );
        for t in [UiTab::Mind, UiTab::Generator, UiTab::Synth, UiTab::Audio] {
            assert!(!Edition::Console.shows_tab(t), "{t:?} should be hidden in Organon Console");
        }
    }
    ...
    fn default_build_is_the_full_edition() {
        ...
        assert_eq!(EDITION, Edition::Console);
    }
}
