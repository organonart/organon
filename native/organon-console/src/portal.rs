...
//! window server can answer. `shell_main.rs` owns the texture, the render and the paint, and
...
//! addition is stated where it is enforced rather than here — see `shell_main.rs`'s
...
impl PortalState {
    /// Is a portal on screen? The one question `shell_main.rs` asks of the state today.
...
mod tests {
    ...
    /// `Shell` is constructed with has to be `Closed` without anyone remembering to say so.
    #[test]
