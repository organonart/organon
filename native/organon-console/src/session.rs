//! The Console session/event model — the shared vocabulary (Console #4 Tier 1).
//!
//! The PRD §5 objects as serde types, a [`SessionEvent`] envelope, and an
//! append-only JSONL log with torn-tail recovery. Every other Console tree builds on
...
/// A live or static view presented in the Console (viewport, diff, plot, …).
#[derive(Debug, Clone, PartialEq, Serialize, Deserialize)]
pub struct Surface {
    pub id: String,
...
/// A versioned, reviewable addition to the Console or Organon (PRD §5 "Extension").
...
///
/// ⚠️ **Contract.** Console #5 (command service) and #7 (agent bridge) build
...
impl SessionLog {
    /// The Console store root: `dirs::data_dir()/OrganonShell` — the one-resolver
...
mod tests {
    ...
    /// The drift guard for Console #5/#7: the exact serialized field names and
