...
//! `substrate_materials`, and should not: the caller (`console_main.rs`) fills the labels from
...
/// them apart is what stops `organon-console` from acquiring an opinion about a command-line
...
// `console_main` wears the same hairline around the portal, so a second set of constants
