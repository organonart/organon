//! The Console compositor skeleton — the five PRD §2 regions as egui panels.
//!
//! Layout (Console #3 T1):
//!
//! ```text
...
//! ```
//!
//! Every decision is a pure function of [`ConsoleApp`] state + [`Edition`], never of
...
/// The embedded live Organon viewport, as the UI sees it (Console #6 T1).
///
...
/// surfaces) belongs to Console #4's schema, which this struct will consume —
/// growing fields here that #4 owns would be the two-vocabularies mistake one layer
/// down.
pub struct ConsoleApp {
    /// Which product this front-of-house is dressed as. Always `Edition::Console` in
    ...
    /// The live viewport, once the bin has one to show (Console #6 T1). `None`
    ...
    /// The scripted demo agent, when one is running (Console #7 T1). Kept
...
impl ConsoleApp {
    ...
    /// Console #4's Project object exists).
...
/// Run one frame of the Console UI. The caller owns the `egui::Context` and the frame
/// loop (`src/main.rs` for the product; a bare context in tests) — and the [`Theme`],
/// which is borrowed here rather than held on [`ConsoleApp`] so that the palette has exactly
/// one owner in a process however many front-ends are on screen.
pub fn ui(ctx: &egui::Context, app: &mut ConsoleApp, theme: &Theme) {
    ...
    egui::TopBottomPanel::bottom("evidence-strip")
        ...
        .show(ctx, |ui| {
            ui.horizontal_centered(|ui| {
                ui.weak("Evidence · command runs · captures · delegated work — Console #4");
            });
        });
...
/// The live Organon viewport filling the central workspace (Console #6 T1).
///
...
fn top_bar(ctx: &egui::Context, app: &mut ConsoleApp) {
        ...
        ui.horizontal_centered(|ui| {
            ...
            ui.with_layout(egui::Layout::right_to_left(egui::Align::Center), |ui| {
                ...
                let _ = ui.button("Ask Pi").on_hover_text(
                    "The primary agent connects in Console #7",
                );
...
fn command_palette(ctx: &egui::Context, app: &mut ConsoleApp) {
...
mod tests {
    ...
    fn heading_names_the_product() {
        let console = ConsoleApp::new(Edition::Console);
        assert!(console.heading().starts_with("Organon Console"));
        ...
        let full = ConsoleApp::new(Edition::Full);
        assert!(full.heading().starts_with("Organon ·"));
    }

    #[test]
    fn palette_toggle_clears_stale_query() {
        let mut app = ConsoleApp::new(Edition::Console);
        app.toggle_palette();
    ...
    fn ui_runs_headless_for_every_edition() {
        for edition in [Edition::Console, Edition::Full, Edition::Mind] {
            let ctx = egui::Context::default();
            let mut app = ConsoleApp::new(edition);
            app.palette_open = true; // cover the palette path too
            let _ = ctx.run(egui::RawInput::default(), |ctx| ui(ctx, &mut app, &Theme::organon()));
        }
    }

    /// The viewport pane path (Console #6 T1), with a dummy texture id — no GPU
    ...
    fn frame_at(app: &mut ConsoleApp, t: f64) {
        let ctx = egui::Context::default();
