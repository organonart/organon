...
pub enum Lane {
    /// Handed down by `shell_main`: dispatched onto the console's audited sidecar, exactly
...
impl Registry {
    ...
    /// table is pinned collision-free by a test in `shell_main`, which is the only place that
    /// can see it.
