...
- 🚨 **THE CONSOLE HAS NEVER BEEN RUN ON macOS. Not once, by anyone.** No window has opened
  ...
  ✅ **What is measured, from a Linux cloud container with no Mac anywhere in reach.** Every
  nih_plug-free workspace member type-checks clean for **`aarch64-apple-darwin`**, `--all-targets`
  (lib *and* tests), exit 0 each: `organon-core`, **`organon-console`**, `organon-scene`,
  `organon-render`, `organon-mind`. That is the compositor — the bulk of the Console — compiler-verified
  for Apple silicon rather than assumed. `platform.rs` is why it costs nothing to believe: macOS
  folds into `Platform::Unix`, its `/bin/zsh` fallback was written for a Mac, and both arms are
  exercised by tests on every host, so a Mac-shaped launch decision is not a `#[cfg]` nobody can run.
  ...
  (a `-p` list of members is exactly the hand-maintained thing the `--workspace` rule forbids)
  and what the leg does not prove.
  🚨 **What a green `build (macos)` will and will not mean.** It will mean *green and ready to
  deploy* — it compiles, and the workspace suite passes on macOS. It will **not** mean verified
