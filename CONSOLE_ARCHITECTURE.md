...
|---|---|---|---|---|
| 1 | 27.6 µs | 12.4 µs | 0.05 µs | 0.06 ms → 0.000 ms |
| 10 | 33.9 µs | 14.2 µs | 0.05 µs | 0.37 ms → 0.001 ms |
| 100 | 109.9 µs | 13.0 µs | 0.05 µs | **11.10 ms** → 0.005 ms |
...
**drawn**, so this is per **frame**, not per keystroke. Straight from disk that is 11.1 ms against
a 16.7 ms frame, and it would be spent while somebody is typing. ⚠️ **A `stat`-per-call cache is
not the fix either** — 13 µs × 101 is still 1.3 ms, several times a frame.
