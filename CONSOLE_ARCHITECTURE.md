...
>
> ⚠️ **Those two annex filenames still say `shell` because that is what the files are
> actually called.** They live in a tree this repo cannot see, so renaming the citation
> here would only dangle it — the same reasoning as the two exclusions below. Rename the
> annex first, then these.
>
> **`Console #N` in this file, and in the crate's doc comments, is a Console work item** —
> a tree in the tracker the product was planned in, where it was called *Shell*. The
> number is the provenance and is unchanged; only the word moved. Bare `#N` means this
> repo's tracker. Both are kept as provenance; neither is a link.
>
> **Not auto-injected.** Open it deliberately when working on the Console, like
> `MIND_ARCHITECTURE.md` and `doc/arch/render.md`.
>
> ⚠️ **The product is Organon Console everywhere in this tree — with exactly two
> exceptions, and both are read from OUTSIDE this repo.** The crate is
> `native/organon-console`, the feature is `console-edition`, the binary module is
> `native/src/console_main.rs`, the app state is `Console`, this file is
> `CONSOLE_ARCHITECTURE.md`. What did **not** move:
>
> 1. **The IPC namespace value `"organon-shell"`** (`organon-core/src/edition.rs`). The
>    `organon` CLI joins on this exact string to find a running console, and the launch
>    shims set `ORGANON_IPC_NS=organon-shell` to fork a second one. It is a wire
>    identifier, not a name; a test pins it.
> 2. **The `ORGANON_SHELL_*` environment variables** — all nine. They are a shipped flag
>    surface that this machine's `organon-console.cmd` / `oc.cmd` shims already set.
>
> Renaming either would break the far side with no error on either end. Everything else
> renamed precisely because nothing outside this repo reads it.
...
- **Process launching is platform data, not `#[cfg]` (`platform.rs`, 2026-08-08)** —
  ...
  ⚠️ **This exists because the Console shipped unable to open a shell on Windows while the
...
- **WSL harnesses (Windows)** — `HarnessSpec` gains `cwd`, `wsl`, `wsl_distro`;
  ...
  **One rule governs `cwd`: it is a path in the namespace the process actually STARTS
  ...
  fail in the other.
- **The self-steering loop (#14 T1)** — the Console publishes the default-look `Shared`
  ...
  the `Shared` seqlock for motion; the Console bumps it once per redraw, so a console
...
**Nothing selects a palette yet**: no picker, no CLI verb, no startup read. `console_main.rs`
...
while one is being chosen becomes a rewrite instead of a second value. `ConsoleApp` gets no
...
`console_main.rs` calls `load_default()` yet — wiring it belongs with the consumer, because a
...
`Posture::TERMINAL` and nothing moves it**: `Console` sets the field once and holds it, so the
...
**One owner, no globals** — `Console` holds `posture: Posture` beside its `Theme`, `redraw`
...
- ⚠️ **The desktop posture has never been drawn, and neither has the terminal one since the
  ...
  `cargo test -p organon-console --lib` is **513 green** (500 on `main`, plus twelve in
  `posture` and one in `theme`) and `cargo check --features console-edition --bin
...
- ⚠️ **No preferences file has ever been written on a real machine.** §1.5 is pinned by ten
  ...
  `SessionLog::store_root()`. `cargo test -p organon-console --lib` is 490 green and `cargo
  check --features console-edition --bin organon-console` is clean. **That is the whole
...
- 🚨 **Nobody has ever seen `light`, `dark` or `chocolate`, and a palette that passes its hex
  ...
  the entire claim is: `cargo test -p organon-console --lib` is **508 green** (500 before, eight
  new tests) and `cargo check --features console-edition --bin organon-console` is clean. **It
...
- **The backdrop is the DEFAULT LOOK of the engine**, not a live external system:
  the Console writes the default `Shared` itself and the CLI's override lane mutates the
...
- The CLI op-path can print a cosmetic "queued" warning in-console — the ops drain
  ...
  Feedback channel, and the Console **does** write `Shared` each redraw
...
- ✅ **The terminal's byte path is now MEASURED on Windows — it was a missing DSR
  ...
  or (b) the Console's reader not draining ConPTY, a product bug. **Both were wrong.** A
