...
`shell_main` for the reason the button and slider tables are: the vocabulary is built from
...
next diagnostic replaces it. `shell_main`'s table is asserted collision-free, so this path is
...
### 1.6 The re-wrap measurement — what a width change costs the transcript
...
⚠️ **There is no posture module and no §1.6-that-describes-posture.** Posture (issue #38) is
designed and unbuilt, and this section is not a stand-in for it. What exists is the number its
tween is constrained by, taken before the code rather than after — the order the portal's
"immersive is nearly free" claim did not get.
...
| Posture's tween, and pane splitting | Both change the transcript's available width, and **the cost of that is now measured rather than assumed** — §1.6, in full at `doc/console_rewrap_measurement.md`, with five priced options and no decision taken. The two things the design has to answer before either is scoped: whether the tween moves the *wrap width* at all (option B holds it fixed for free), and whether the scrollback is virtualised first (option E, the only one that also fixes the steady-state cost §1.6 found underneath). ⚠️ Do not scope a smooth 0 → 90 pt tween against a ten-card transcript — the number that decides it is the 2 000- and 10 000-element row | #38 · `console_view_paradigm.md` §2, §9 |
