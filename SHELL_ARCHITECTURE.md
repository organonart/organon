...
🚨 **`shell_main.rs` called `egui_ctx.set_visuals(egui::Visuals::dark())` once, hardcoded, and
...
rather than as light. `Theme::visuals()` derives `Visuals` from the palette; `shell_main.rs`
...
**This build ships at `Posture::TERMINAL` and nothing moves it**: `Shell` sets the field once
and holds it, so the tier adds the axis without moving along it. ⚠️ The desktop end has now
been seen once, and it is the reason the margin is symmetric — see the ledger for exactly what
was seen and what still has not been.
