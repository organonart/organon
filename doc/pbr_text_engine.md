...
### 10.1 What shipped, rung by rung (T14, measured against `main @ 997cd96`)

All six are factory presets in `preset.rs::builtin_text_presets`, seeded under the
`seeded_text_v3` marker, named literally so `preset load nixie` needs no guessing. Every rung
shares the room the first GPU look settled — held camera, orbit off, atmosphere off, background
hidden, IBL at a sheen, FX pass on — and each departs in one direction. ⚠️ **None has been looked
at on a GPU**; per rung, what to look for is in the changelog fragment. The rule this section
keeps: a rung sets **only knobs that reach its draw**. A knob set where nothing reads it is a
preset lying about what it does, so where §10's table names machinery that does not reach the
rung, the rung says so here rather than setting it anyway.

| Rung | Shipped as | What is missing, and where it lives |
|---|---|---|
| **`faceplate`** | T3/T9/T10 — unchanged | the brush and the warm rim (§15's table) |
...
and **off** for the two webs (a node per cell is a lattice); glyphs-as-lights **off** for the two
webs (the impostor path empties `instances`, so there is nothing to lower). TAA cannot be
carried by any of them — `temporal[0]` is param-only, said once in T3 and still true.
...
- **T8 — the tracer sees emission.** Every `rt_*` pass and `rt_pathtrace` bind and read the
  ...
  landing surface's emission plays no part — *emitters as photon sources* is its own tier (**T8b**,
  below), and so is NEE toward emitters (the tracer has no light list; it reaches key + fill
  only). Green and
  ready to try; the GPU look this needs is the one §15's row names: the dwell converging lit.
- **T8b — emitters as photon sources. Landed.** The tier T8's layout comment named and left
  undone. `rt_caustic` now binds `emit_buf` at the `@binding(6)` that comment reserved — and
  still does not shade with it: a photon's transport is the *landing* surface's BSDF, so the
  shader has no `instance_emission` and its deposit is still throughput × albedo (pinned by
  test). It reads emission to decide where a photon **starts**. Because the tracer has no light
  list, *lit tile → glass/lens → floor* is the one path a camera-first walk essentially never
  finds; this traces it from the other end. `cs_cdf` builds a per-frame inclusive CDF over the
  live emissive instances in emitted **power** (Φ = π·A·L, L = `emit.rgb * emit.w` — the same
  product `cube.wgsl` adds, so a tile twice as bright throws twice the photons) in one
  workgroup, two passes over the live instances, no readback; `cs_photon` then draws its source
  from the key light or a tile in proportion to power, every photon carrying the same flux
  `(key_power + emitter_power) / N` so each source deposits exactly its own power. The
  ...
  floor, and an Organon scene with no ring looking exactly as it did. `doc/arch/render.md`
  ("The ray-traced passes read it too") carries the mechanism.
...
- **T14 — the preset ladder. Landed** (`main @ 997cd96` + this change): `nixie`, `foundry`,
  ...
  and the knob it is missing. Three things the brief assumed that the tree contradicted, each
  settled by reading the draw rather than the vocabulary: the capsule core reaches **only** the
  capsule impostor draw, so `nixie` (a tile rung) does not set it; there is no thin-film
  `MaterialType`, and the physical film model is Glass-only, so `anodized` is the iridescence
  lobe over a metal; and the two web rungs are honest only because `world.rs` already runs the
  plexus pass *after* the grid lowers — which also means the plexus takes the faceplate tint and
  drops the emission, so both webs are monochrome until that pass reads `emits` (it does,
  as of W17 below). The seed
  gained one rule: a rung name no earlier marker wrote is **never replaced** whatever its
  shape (`TEXT_RUNGS_SEEDED_BEFORE`), because a pre-organon#124 save has no `exposed` set and a
  user's own `nixie` from that era would otherwise read as factory. **Needs a GPU look per
  rung**; none has had one.
- **W17 — the web takes the emission. Landed** (`main @ 1034441` + this change): the one
  line T14 named. While a ring is live the Plexus pass now colours each node from the tile's
  **emission** — `plexus_node_colour(ring_live, tint, emit)` returns `emit.rgb × emit.w`
  (linear radiance, the term `cube.wgsl` adds and T10 ranks by) and, with no ring, the
  generator's tint byte for byte. **It feeds `ntints`, because that is the only lane**:
  `fs_capsule` derives both the albedo and, × the material's glow, the emission from the
  impostor's single `color`, and T6's core shows exactly that emission through the shell;
